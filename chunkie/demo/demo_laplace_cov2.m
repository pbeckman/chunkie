...
for i=1:chnkr.nch
    for j=1:chnkr.k
        ...
        rhs((i-1)*chnkr.k + j) = chnk.lap2d.kern(srcinfo, targinfo, 'sprime');
    end
end
start = tic; mu  = gmres(sys',rhs,[],1e-13,100); t1 = toc(start);
fprintf('%5.2e s : time for dense gmres on K^T \n',t1)
start = tic; sig = gmres(sys, mu, [],1e-13,100); t1 = toc(start);
